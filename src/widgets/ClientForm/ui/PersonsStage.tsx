...
export const PersonsStage = () => {
  ...
  const setPersons = useClientData((state) => state.setPersons);
  const addStage = useStage(addStageSelector);
  const setMainPerson = useClientData((state) => state.setMainPerson);
  const {
    setValue,
    handleSubmit,
    register,
  ...
  const mainPerson = useClientData((state) => state.mainPerson);
  const persons = useClientData((state) => state.persons);
  useEffect(() => {
    if (mainPerson) {
